...
void DirectionalAntennaImprove:: initAllSU()
{
    ...
    for (int i = 4; i < 6; i++) {
        MySU &su = allSU[i];
        su.allChannelNum = (int)(global::TOTAL_CHAN_NUM * this->ALL_CHANNEL_RATIO);
        su.allChannelToHop = CRmath::generateDiffRandInt(su.allChannelNum, 1, global::TOTAL_CHAN_NUM);
...
void DirectionalAntennaImprove::getAvaiChanEachSectorNewSU(MySU &su, int t, int _sector)
{
    su.avaiChan.clear();
