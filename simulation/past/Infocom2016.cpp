...
void Infocom2016::energySavingSameSeq(int t) {
    ...
    bStrStartPos0++;
    if (bStrStartPos0 == SEND_REV_STR_LEN) bStrStartPos0 = 0;
    bStrStartPos1++;
    if (bStrStartPos1 == SEND_REV_STR_LEN) bStrStartPos1 = 0;
}
...
void Infocom2016::SendRevSeqRend(int t) {
    ...
    if (abs(c0 - c1) == global::TOTAL_CHAN_NUM && vectorSortedFind(su0.avaiChan, min(c0, c1)) != -1 &&
        vectorSortedFind(su1.avaiChan, min(c0, c1)) != -1) {
        rendSuc[1] = 1;
        totalAnswer[1] += t - START_REND_TIME;
...
void Infocom2016::pureRandomRend(int t) {
    ...
    int t0 = CRmath::randIntDiff(0, 1);
    int t1 = CRmath::randIntDiff(0, 1);
    if (c1 == c0 && (t1 ^ t0) == 1 && vectorSortedFind(su0.avaiChan, c0) != -1 &&
        vectorSortedFind(su1.avaiChan, c1) != -1) {
        rendSuc[2] = 1;
        totalAnswer[2] += t - START_REND_TIME;
